...
pub trait Hashable {
    ...
    fn hash(&self) -> H256;    // hash() 函数，返回一个H256值，相当于规定一个函数的输入与输出类型
}

/// A SHA256 hash.
#[derive( Hash, Eq, PartialEq, Serialize, Deserialize, Clone, Default, Copy)]
pub struct H256([u8; 32]); // big endian u256

#[derive( Hash, Eq, PartialEq, Serialize, Deserialize, Clone, Default, Copy)]
pub struct H160([u8; 20]);

impl Hashable for H256 {            // 函数具体实现：输入与输出确定，计算方法规定：此实现是关于H256化一个对象
    fn hash(&self) -> H256 {
...
pub fn convert_to_h160(num: H256) -> H160 {

    let num_u8:[u8;32] =From::from(num);
    let mut num_160:[u8;20] = [0;20];
    for i in 0..19 {
        num_160[i] = num_u8[i+12];
    }
    num_160.into()
}
